...
const main = async (event) => {
  ...
  // Get the hours interval from the event pattern
  const hours_interval = event.detail.hours_interval;    
  ...
  try {
    ...
    for (const cameraName of cameras) {
      ...
      if (imagesFolderExists) {
        ...
        const imageKeys = await listImageKeys(IMAGES_FOLDER, hours_interval);
        ...
        const videoKey = `${FOOTAGE_FOLDER}${cameraName}-${timestamp}-${hours_interval}_hours_.mp4`;
...
async function getCameraFolders() {
  ...
  for (const cameraId of allowedCameras) {
    ...
    if (data.Items.length === 0) {
      ...
      const camera_name = data.Items[0].camera_name.S; // Retrieve camera name from the data.Items array
      cameras.push(camera_name);
    }
  }
  return cameras;
}
...
async function listImageKeys(folder, hours_interval) {
      ...
      do {
        const response = await s3.listObjectsV2(params).promise();
        const now = new Date();
        const startOfHour = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours()- hours_interval, 00, 0);
...
async function saveVideo(videoKey, cameraName, FOOTAGE_FOLDER) {
  ...
  const time_stamp = moment().utcOffset(480).format('YYYY-MM-DD-HH-MM-SS');
  const params = {
    Bucket: BUCKET_NAME,
    Key: videoKey,
    Body: fs.createReadStream(videoPath),
    ContentType: 'video/mp4',
    Tagging: `camera=${cameraName}&interval_hours=${hours_interval}&timestamp=${time_stamp}`,
  };
